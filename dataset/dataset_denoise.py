...
import torch
sys.path.append('Uformer/')
from utils import is_png_file, Augment_RGB_torch
...
class DataLoaderTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        clean = torch.from_numpy(np.float32(Image.open(self.clean_filenames[tar_index]))).reshape(256,256,1)/255
        clean = clean.repeat(1,1,3)
        noisy = torch.from_numpy(np.float32(Image.open(self.noisy_filenames[tar_index]))).reshape(256,256,1)/255       
        noisy = noisy.repeat(1,1,3)
        #print(clean.shape)
...
class DataLoaderTest(Dataset):
    ...
    def __getitem__(self, index):
        ...
        inp = TF.to_tensor(inp).reshape(1,256,256)/255
