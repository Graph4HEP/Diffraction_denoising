...
class CustomDataGenerator(tf.keras.utils.Sequence):
    ...
    def __augment_data_with_pos(self, lc, hc, id):
        if id == 0:
            # Flip both LC and HC left-right
            lc_aug = np.fliplr(lc)
            hc_aug = np.fliplr(hc)
...
def get_dataframes_separate_datasets(train_folder, valid_folder, save_folder, batch_size, target_shape, shuffle=True, simulate_lc=None, augmentation_train=False, augmentation_valid=False):
    ...
    if simulate_lc['type'] is None:
        ...
        data_valid = CustomDataGenerator(df=valid_df, batch_size=batch_size*4, target_shape=target_shape, simulate_lc=None, augmentation=augmentation_valid)
    else:
        # Check whether to crop the image (assuming all images in the folder have the same size)
        ...
        data_valid = CustomDataGenerator(df=[fn, "simulated_lc_valid"], batch_size=batch_size*4, target_shape=target_shape, simulate_lc=simulate_lc['type'], augmentation=augmentation_valid, scale_range=[10000,10001])
